...
- No guaranteed ordering across content types (no `chain()`); correctness is not currently dependent on type ordering.
...
- `algolia_aggregation_keys = algolia_client.get_aggregation_keys_for_catalog_query(catalog_query_id, index_name=index_name)`
...
- If `force=False`:
  - Filter to stale or never-indexed records (courses) and “child-indexed-more-recently” logic (programs/pathways) is *not* required here because the set is already “membership target”.
