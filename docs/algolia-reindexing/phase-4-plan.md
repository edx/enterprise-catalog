...
- No enforced dependency ordering across content types (no `chain()`/wait semantics) for now; ordering is not currently required for correctness.
...
- For each content type, chunk the final key list and enqueue the corresponding batch task:

  - `index_courses_batch_in_algolia.delay(content_keys=batch, force=force, index_name=index_name)`
  - `index_programs_batch_in_algolia.delay(...)`
  - `index_pathways_batch_in_algolia.delay(...)`

No `chain()` is used; tasks are simply enqueued.
